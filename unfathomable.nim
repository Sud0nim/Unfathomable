...
proc `+` *(a, b: Area): Area =
  if a.units == b.units:
    Area(size: a.size + b.size,  units: a.units)
  else:
    Area(size: a.size + b.sizeAs(a.units),  units: a.units)
...
proc `*` *(a: Area; b: float): Area =
  Area(size: a.size * b, units: a.units)

proc `*` *(a: float; b: Area): Area =
  Area(size: b.size * a, units: b.units)
